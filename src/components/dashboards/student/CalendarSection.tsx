...
import { Calendar, ChevronLeft, ChevronRight, BookOpen, Flag, Star, AlertCircle, Coffee, Music, Clock, Bell } from 'lucide-react';
import { supabase } from '../../../lib/supabase';
import type { EventRow, EventType } from '../../../lib/supabase';

/* ── Brand-aligned event colours ── */
...
export default function StudentCalendarSection() {
  ...
  const upcoming = filtered.filter(e => e.start_date >= today).slice(0, 12);
  const thisWeek = events.filter(e => {
    const days = daysUntil(e.start_date);
    return days >= 0 && days <= 7;
  });

  /* Build a set of dates that have events in the current viewed month */
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">School events, holidays, exams & activities — 2025/2026</p>
        </div>
        {thisWeek.length > 0 && (
      ...
      <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
        ...
        <div className="xl:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          {/* Month Nav */}
          <div className="bg-gradient-to-r from-indigo-700 to-indigo-600 px-5 py-4 flex items-center justify-between text-white">
            <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-white/20 transition-colors">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <h3 className="font-bold text-sm tracking-wide">{MONTH_NAMES[viewMonth]} {viewYear}</h3>
          ...
          <div className="grid grid-cols-7 bg-indigo-50">
            {DAY_HEADERS.map(d => (
              <div key={d} className="py-2 text-center text-[10px] font-bold text-indigo-600 uppercase">{d}</div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-7 p-3 gap-0.5">
            {calCells.map((day, i) => {
              ...
              const hasEvents = day ? eventDatesInView[day] : undefined;
              return (
                <div
                  key={i}
                  className={`aspect-square flex flex-col items-center justify-center rounded-lg text-xs font-medium transition-all ${
                    ...
                    hasEvents ? 'bg-indigo-50 text-indigo-700 cursor-pointer hover:bg-indigo-100' :
                      ...
                      {hasEvents && !isToday && (
                        <div className="flex gap-0.5 mt-0.5">
                          {hasEvents.slice(0, 3).map((t, j) => (
                            <span key={j} className={`w-1 h-1 rounded-full ${EVENT_META[t].dot}`} />
                          ))}
                        </div>
                      )}
                    </>
          ...
          {/* Legend */}
          <div className="px-4 py-3 border-t border-gray-50 flex flex-wrap gap-x-4 gap-y-1">
            {(Object.entries(EVENT_META) as Array<[EventType, typeof EVENT_META[EventType]]>).map(([type, meta]) => (
        ...
        <div className="xl:col-span-3 space-y-4">
          ...
          {thisWeek.length > 0 && (
            <div className="bg-gradient-to-r from-indigo-700 to-indigo-600 rounded-2xl p-4 text-white">
              <p className="text-xs font-semibold opacity-80 mb-3 flex items-center gap-1.5">
                  ...
                  return (
                    <div key={e.id} className="flex items-start justify-between gap-3 bg-white/15 rounded-xl px-3 py-2.5">
                      <div className="flex items-start gap-2 min-w-0">
                        {(() => { const Icon = EVENT_META[e.event_type].icon; return <Icon className="w-4 h-4 mt-0.5 flex-shrink-0 opacity-90" />; })()}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-50 flex items-center gap-2">
              ...
              <h3 className="font-bold text-gray-800 text-sm">
                {filter === 'all' ? 'All Upcoming Events' : `Upcoming — ${EVENT_META[filter].label}`}
              </h3>
              {upcoming.length > 0 && (
                <span className="ml-auto text-xs text-gray-400">{upcoming.length} event{upcoming.length !== 1 ? 's' : ''}</span>
              )}
            </div>
                ...
                return (
                  <div key={e.id} className="flex items-start gap-3 px-5 py-3.5 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-indigo-50 flex flex-col items-center justify-center border border-indigo-100">
                      <span className="text-[10px] text-indigo-500 font-semibold leading-none uppercase">
                        {new Date(e.start_date).toLocaleDateString('en-NG', { month: 'short' })}
                      </span>
